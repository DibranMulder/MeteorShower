class Player {
    ...
    public paint(animationAgeInMs: number) {
        ...
        if (socket && socket.id) {
            socket.emit("position-changed", socket.id, this.currentDirection, this.jumping, pointerManager.isMoving, this.playerAnimation.position.x, this.playerAnimation.position.y);
        }
